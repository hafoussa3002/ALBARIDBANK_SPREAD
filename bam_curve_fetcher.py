...
def _direct_csv_url_for_date(d: date) -> str:
    return f"{DIRECT_CSV_URL}?date={quote(d.strftime('%d/%m/%Y'))}"
...
class BamCurveFetcher:
    ...
    async def _fetch_one_date_async(self, session: aiohttp.ClientSession, d: date) -> tuple[date, tuple[list[int], list[float]] | None]:
        ...
        # 3) Mark no-data (weekend/holiday/not available)
        self._save_no_data_marker(d)
        return d, None

    async def _async_get_direct_csv_with_retry(self, session: aiohttp.ClientSession, d: date) -> Optional[str]:
        url = _direct_csv_url_for_date(d)
        return await self._async_get_text_with_retry(session, url=url)
    ...
    async def _async_fetch_via_html_with_retry(self, session: aiohttp.ClientSession, d: date) -> Optional[str]:
        ...
        for params in payloads:
            html = await self._async_get_text_with_retry(session, url=BASE_URL, params=params)
            if not html:
                continue
    ...
    def _sync_fetch_direct_csv(self, d: date) -> Optional[str]:
        url = _direct_csv_url_for_date(d)
        return self._sync_get_text_with_retry(url=url)
    ...
    def _sync_fetch_via_html(self, d: date) -> Optional[str]:
        ...
        for params in payloads:
            html = self._sync_get_text_with_retry(url=BASE_URL, params=params)
            if not html:
                continue
