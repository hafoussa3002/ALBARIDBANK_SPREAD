...
def _get_credentials() -> tuple[str, str]:
    try:
        ...
        code = st.secrets["APP_CODE"]
    except Exception:
        user = "spreadABB"
        code = "albarid2026"
    return str(user), str(code)
...
def _render_login(logo_path: Optional[Path]) -> None:
    ...
    with c2:
        ...
        if submit:
            app_user, app_code = _get_credentials()
...
def _compute_taux_bdt(
    ...
    fetcher = _get_fetcher(cache_dir)
    issue_dates = [d for d in df["ISSUEDT_DT"].dt.date if pd.notna(d)]

    def _prog(done: int, total: int, n_cached: int, n_fetch: int) -> None:
        pct = min(done / max(total, 1), 1.0)
        progress.progress(pct, text=f"Cache: {n_cached} | Reseau: {n_fetch} | {done}/{total}")

    curves = fetcher.get_curves_parallel(issue_dates, max_workers=max_workers, progress_callback=_prog)
    ok_dates = [d for d, v in curves.items() if v is not None]
    ko_dates = [d for d, v in curves.items() if v is None]
...
def main() -> None:
    ...
    if st.button("Lancer pricing", type="primary"):
        ...
        if ko_dates:
            with st.expander(f"⚠️ {len(ko_dates)} dates sans courbe BAM — cliquer pour voir"):
                ko_df = pd.DataFrame({"Date manquante": sorted(ko_dates)})
                code_col_ko = _find_code_column(dff_rates.columns.tolist())
                if code_col_ko is None:
                    dff_rates["CODE_FALLBACK"] = pd.NA
                    code_col_ko = "CODE_FALLBACK"
                ko_isin = dff_rates[dff_rates["Taux BDT"].isna()][[code_col_ko, "ENGLONGNAME"]].drop_duplicates()
                ko_isin = ko_isin.rename(columns={code_col_ko: "CODE", "ENGLONGNAME": "DETAILS DU TITRE"})
                st.dataframe(ko_df, use_container_width=True)
                st.dataframe(ko_isin, use_container_width=True)
        st.write(
            {
        ...
        chart_df = out_df.dropna(subset=["Spread"]).copy()
        if not chart_df.empty:
            fig1 = px.bar(
                chart_df.groupby("BANQUE", as_index=False)["Spread"].mean().sort_values("Spread", ascending=False),
                x="BANQUE",
                y="Spread",
                title="Spread moyen par banque (bps)",
                color="Spread",
                color_continuous_scale="RdYlGn_r",
                labels={"Spread": "Spread moyen (bps)", "BANQUE": "Banque"},
            )
            fig1.update_layout(paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)", font_color="#f6f1e9")
            ...
            fig2 = px.histogram(
                ...
                x="Spread",
                nbins=30,
                title="Distribution des spreads (bps)",
                color_discrete_sequence=["#e3c100"],
            )
            fig2.update_layout(paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)", font_color="#f6f1e9")
            st.plotly_chart(fig2, use_container_width=True)

            fig3 = px.scatter(
                chart_df.dropna(subset=["Maturite residuelle"]),
                x="Maturite residuelle",
                y="Spread",
                color="BANQUE",
                title="Spread vs Maturite residuelle",
                labels={"Maturite residuelle": "Maturite (annees)", "Spread": "Spread (bps)"},
            )
            fig3.update_layout(paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)", font_color="#f6f1e9")
